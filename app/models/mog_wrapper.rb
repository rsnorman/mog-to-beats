...
class MogWrapper
	...
	def login(username, password)
		...
		@mogid = self.agent.cookies.detect{|x| puts x.name; x.name == 'mogid'}.value rescue nil
		@is_logged_in = self.agent.page.uri.to_s == "https://mog.com/m" && @mogid
	end

	def get_playlists
		response = RestClient.get("https://mog.com/v2/playlists/user.json?ts=1395719796&api_token=#{@mogid}&allow_nonstreamable_token=1")
		
		playlists = []

		JSON.parse(response.body)["playlists"].each do |playlist|
			url = "https://mog.com/v2/playlists/#{playlist["playlist_id"]}.json?ts=1395720541&api_token=#{@mogid}&allow_nonstreamable_token=1"
	...
	def get_playlist_tracks(playlist)
		response = RestClient.get(playlist.url)
	...
	def get_favorite_artists
		page = get_favorite_artist_page
		artists = []

		while artists.size < page["total"]
			artists = artists.concat(page["artists"])
			page = get_favorite_artist_page(page["count"] + page["index"])
		end

		artists = artists.collect do |t|

			artist = Artist.find_by_mog_id(t["artist_id"])
	...
	def get_favorite_artist_count
		get_favorite_artist_page["total"]
	end

	private

	def get_favorite_track_page(index = 0, size = 100)
		response = RestClient.get("https://mog.com/v2/bookmarks/tracks.json?explicit=1&index=#{index}&count=#{size}&ts=1395794345&api_token=#{@mogid}&allow_nonstreamable_token=1")
		JSON.parse(response.body)
	end

	def get_favorite_album_page(index = 0, size = 100)
		response = RestClient.get("https://mog.com/v2/bookmarks/albums.json?explicit=1&index=#{index}&count=#{size}&ts=1395794345&api_token=#{@mogid}&allow_nonstreamable_token=1")
		JSON.parse(response.body)
	end

	def get_favorite_artist_page(index = 0, size = 100)
		response = RestClient.get("https://mog.com/v2/bookmarks/artists.json?explicit=1&index=#{index}&count=#{size}&ts=1395794345&api_token=#{@mogid}&allow_nonstreamable_token=1")
		JSON.parse(response.body)
	end
